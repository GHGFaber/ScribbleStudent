...
function CreateClass({ onClose, classes }) {
    ...
    const createClass = async(req, res) => {
        try {
          ...
          if (classVerified) {
            ...
            const confirmed = window.confirm(`Are you sure you want to create the class with the following details?\nClass Name: ${className}\nDescription: ${description}`);
            if (confirmed) {
    ...
    const verifyClassName = async(req, res) => {
      try {
        ...
        const classExists = data.classData.some(item => item.className === className);
        if (classExists) {
